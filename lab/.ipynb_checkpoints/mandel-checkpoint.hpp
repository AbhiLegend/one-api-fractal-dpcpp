...
struct MandelParameters {
  ...
  // Variation function to mutate mandelbrot
  static ComplexF variation(ComplexF z, ComplexF c, int option)
  {
      switch (option) {
      case 1:
          return sin((z)+c);
...
int max_iterations() const { return max_iterations_; }
  ...
  int Point(const ComplexF &c, int option) const {
    int count = 0;
    ComplexF z = 0;
...
class MandelParallel : public Mandel {
 public:
  ...
  void Evaluate(queue &q, int option) {
